...
void FragTrap::vaulthunter_dot_exe( std::string const & target )
{
	...
	if (this->_energyPoints > 24)
	{
		...
		std::cout << "FragTrap attacks " << target
				  << " with " << weapons[std::rand() % 5] << " causing "
				  << damage[std::rand() % 5] << " points of damage! Energy: " <<
				  this->_energyPoints << std::endl;
		std::cout << "FragTrap: " << "\"Oh god I can't stop!\"" <<
		std::endl;
	}
	else {
		std::cout << "FragTrap don't have energy for fight! Energy: " <<
			  this->_energyPoints << std::endl;
		std::cout << "FragTrap: " << "\"Crap happens.\"" << std::endl;
	}
